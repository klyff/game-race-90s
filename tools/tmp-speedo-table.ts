...
// device-detection code dereferences `window` as a side effect of that import. There is
// no DOM in plain Node, so importing anything from that file — even a pure, Phaser-free
// function — throws `ReferenceError: window is not defined` unless something shims a
// minimal `window` first. This shim exists ONLY so this throwaway script can import the
// real exported helper (rather than re-implementing its maths and risking drift); it is
// not a fix and is not part of the shipped change.
