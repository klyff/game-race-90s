...
| T-050 | Jump ramps: elevation + airtime | `in_progress` | main (orchestrator) | — | **New scope, owner, 2026-08-16, same message as T-048: "mais phicica, pista com rampa pra pular."** No z-axis exists anywhere in the domain today (`Vec2`, `VehicleState`, `TrackDefinition` are strictly 2D), but `IsoProjection.toScreen(point, height)` already supports a height parameter (`ISO_Z = 0.85`), proven by `ExplosionEffect`'s rising fireball — only the physics side is missing. Plan: new `RampZone` data (`triggerDistance`, `triggerLength`, `launchSpeed`) as an optional `TrackDefinition.rampZones`; `VehicleState` gains `height`/`verticalVelocity`; gravity integration + wall/grip suspension while airborne lives in `OnTrackStep.stepVehicleOnTrack`; car-to-car contact skipped between an airborne car and anything else in `RaceField`; `VehicleView.sync` passes height through to the projection with the shadow staying grounded. Explicit exception to decision 19 (walls): airborne skips wall resolution entirely rather than repurposing it — call this out, don't quietly reuse `yawSpin` or the wall-scrape path. See plan file for the full design, including the ~8-file `VehicleState` migration blast radius and the risks section (weapons/hazards still hit by 2D position only; depth-sort ignores height; ramp landings must stay inside `trackFullHalfWidth`). |
