...
| T-042 | **Championship structure: 3 tracks per world, ordered by difficulty, with a points gate to the next world** | `todo` | — | T-041, T-036 | **New scope from the owner, 2026-08-16, explicitly deferred: "isso é para o próximo plano".** Verbatim intent: three tracks per world — 1 easy, 2 medium, 3 hard — and the player only advances to the next world on **80 points**, where the owner asked ME to work out what 1st/2nd/3rd is worth per track so it sums correctly, and also framed it as "80% do tempo mínimo". Today there is ONE track (`thunder-basin`) and `TRACKS`/`findTrack` is a flat registry with no world grouping, no difficulty ordering and no progression at all. **Proposed maths, to be confirmed with the owner before building — and the reason it needs confirming is that "80 points" and "80% of the minimum time" read as TWO different gates, which would let a player who wins every race still be blocked by a stopwatch.** The recommendation is to reconcile them into ONE 0–100 world score, so "80 points" literally means 80%: position component (70 of the 100) = points summed over the world's three tracks divided by the maximum, with a five-car race paying **1st 10, 2nd 6, 3rd 4, 4th 2, 5th 1** (30 available across three tracks); time component (the other 30) = the mean of `parTime / playerTime` per track, capped at 1, where each `TrackDefinition` carries an authored `parTime`. Three wins with poor times, or two wins and a second with good times, both clear 80 — winning matters most but cannot be the whole story, which is what the owner's two sentences together imply. Needs a `world` grouping and a `difficulty` + `parTime` on `TrackDefinition`, the nine missing tracks (the ten planet illustrations from T-034 are the worlds), and progression persisted through T-035's save. |
