...
30. **NPC AI V2 is the live brain. Do not restore utility-first `RECOVER`.** Shipped `292c0ab`.
    Pipeline: world → corridor futures from live `(s0,d0)` → `stepVehicle` rollout → RaceCore +
    bounded personality bias. `RECOVER` is an orthogonal control mode (`ControlMode.ts`) with
    hysteresis and `reason=`, not a competing tactic. `RivalTraits` must not dive independently
    (`AIDriver(..., undefined)` in `RaceField`). Pre-V2 dumps in `docs/ia/FINDINGS.md` and
    `docs/ia/runs/` are baseline (RECOVER 85–95%); they are not a brief to retune profile numbers.
    Product “V2 waves” (comic, garage, maps) do not own this pipeline. Remaining: ramp locus `s≈280`
    / airborne hang, then personality — not a rewrite of `RacingAgent`.
...
| T-018 | `SplashScene`: splash art + slow-blinking "PRESS SPACE TO ROCK'N THE 90s" + car select on the same screen | `done` | main + blinkclock-test, splash-layout, car-stat-bars | T-015 | **DONE and VERIFIED BY READING FIVE SCREENSHOTS, 2026-08-16.** The game no longer boots into the race: `boot -> splash -> race`. Delivered `src/scenes/SplashScene.ts`, `src/adapters/audio/TitleAudio.ts`, `src/adapters/render/SplashLayout.ts` (pure, 19 tests), `src/adapters/render/CarStatBars.ts` (pure, 12 tests), `tests/adapters/BlinkClock.test.ts` (33 tests), plus `SPLASH`/`SPLASH_ART_KEY`/`UI_ASSET_DIRECTORY` in `sceneKeys.ts`, the splash image in `BootScene`'s second load pass, and `SplashScene` registered in `main.ts`. **704 tests across 29 files, typecheck and build clean.** What the screenshots proved that state inspection could not: the blink is a genuine hard cut (`/tmp/blink_2400.png` prompt ABSENT, `/tmp/blink_2800.png` prompt PRESENT — no tween, driven by `BlinkClock` + `setVisible`); LEFT/RIGHT really swaps the car (`/tmp/select_right.png` shows DIRT DEVIL with a different sprite, a different archetype line and a full-length GRIP bar, matching `CarStatBars`' roster normalisation); SPACE really starts the race with the CHOSEN car (`/tmp/race_after_space.png` — marauder, `AMMO 5/5`, 5th on the grid). **No title is drawn** — the logo and credit are painted into the art. **Two things the images caught and object state would not have:** the prompt looked "missing" in the first screenshot and was in fact simply mid-blink-OFF (the probe found it at (699, 449) `visible: true`, so BOTH readings were needed to tell a bug from the feature); and the car name crowded the painted credit line, fixed by moving `SPLASH_REGION.VOID.top` from 0.28 to 0.32 — the region means "safe to draw over", not "dark". `tools/verify/probe.mjs` now takes a scene key and dumps every object, because it hardcoded `'race'` and reported the splash as a boot failure. **Still unverified and only the owner can close it: the music has never been heard in the game.** `TitleAudio` starts `TitleMusic` on the first key press (a browser silently ignores a resume outside a user gesture) and stops it on scene SHUTDOWN (nothing in Phaser stops a Web Audio graph, so without that hook the title riff plays over the race). `music.html` at the root is now redundant and can be deleted whenever the owner wants. |
...
| T-036 | Per-planet look: `theme` on `TrackDefinition`, and the area-select screen | `done` (row was stale) | Cursor session | T-034 | **Row corrected 2026-08-16 — this had shipped and the table just never caught up.** `src/data/tracks/planetThemes.ts` carries a full `PlanetTheme` (wall/shoulder/tarmac/kerb/marking/chequerDark/ground + `groundKey`/`artKey`) for all 10 planets, `TrackRenderer` reads it, and stand-in ground tiles exist at `public/assets/ground/<slug>.png` (`npm run gen:ground`). **Still open, tracked under the new T-048 below:** 9 of 10 planets have no real select illustration (only `thunder-basin.jpeg` is real art) and the wall/kerb bands are flat colour only, no per-planet texture or trackside prop. |
| T-039 | Tyre marks stopped being drawn after the first corner | `done` | main | — | **Reported by the owner at the wheel with a screenshot, 2026-08-15, and this was a real regression I introduced when T-013 wired five cars in.** Root cause found by reading the code, not guessed: `TyreMarks` held state for exactly ONE car — a single pair of wheel trails — and `RaceScene` began calling `record()` for all five on one instance. Three things broke at once. (1) `ageAndEvictSegments` ran once PER CAR instead of once per frame, so marks aged five times too fast and faded in about 1.2 s instead of 6. (2) The shared "previous point" ping-ponged between different cars, so a segment could be drawn from one car's wheel to another car's wheel, straight across the track. (3) **The one that actually stopped all drawing:** a car that is NOT sliding resets the trail to `null`, and that reset hit the SHARED trail — so it wiped the trail of every car that *was* sliding. With five cars at least one is almost always gripping, so from the first corner onward the trail was cleared every single frame and `previousPoint === null` short-circuited every segment. **Fix:** trails are now a `Map` keyed by car index (`record(carIndex, state, telemetry)`), and ageing moved into a separate `update(deltaSeconds)` that `RaceScene` calls exactly once per frame. Typecheck, 640 tests and the build are clean, and a mark is visible on screen again in `/tmp/tyre_fixed.png`. **SIGNED OFF BY THE OWNER at the wheel, 2026-08-16: "o pneu voltou, ok na T-039".** Closed. **Lesson worth more than the fix: an adapter that holds per-entity state is a landmine the day the entity count goes from one to many, and nothing about the single-car API said "one".** |
| T-040 | One music theme per world | `done` | main (orchestrator) | T-018, T-036 | **Done 2026-08-16.** `MusicPlayer` (`src/adapters/audio/MusicPlayer.ts`) is the node graph extracted out of `TitleMusic`, parameterised by a `MusicScore` (`src/adapters/audio/MusicScore.ts`: tempo, riff, strum/drum patterns, lead lick, a small timbre block). `TitleMusic` is now just `MusicPlayer` played with `TITLE_SCORE`. Ten scores authored in `src/data/tracks/planetMusic.ts`, mood-matched per the brief above (Vulkanis heavier/slower, Cryo Hollow sparse, Bogmire Deep dragging, Neon Kasbah bright/synth-led, etc.) via tempo + timbre + arrangement density (three shared strum/drum pattern shapes mixed per planet) rather than exotic chord theory. `RaceAudio` takes the track's score and plays it through `MusicPlayer` on its own gain, independent of the SFX mix; muted/started alongside the rest. 30 new tests (`tests/data/PlanetMusic.test.ts` + updated `tests/adapters/TitleMusic.test.ts`), 913 total, typecheck and build clean. Commits `25bd7a9` (extraction) and `a118775` (the ten scores + wiring). |
| T-037 | **Every car gets one signature advantage** — a felt mechanic, not a stat delta | `done` | main + perk-plumbing, slipstream, perk-outcome-tests | — | **DONE 2026-08-16, and every perk is proved by a test that measures the OUTCOME rather than a flag.** Owner's call: *"cada um deve ter uma vantagem, hoje está muito fácil para o jogador."* **The perk ID travels as DATA** (`tools/spritegen/cars/*.car.ts` -> `renderCar.ts`'s literal -> `cars.json` -> `parseCarSheet` -> `RacerEntry` -> `RacerRuntime`, where it is resolved ONCE into a profile) while **the tunables stay in code** in `src/domain/vehicle/CarPerk.ts`, so re-tuning never means regenerating a sprite sheet and `cars.json` stays small. `parseCarSheet` THROWS on an unknown perk id rather than dropping it, because a typo would otherwise become a car that quietly has no advantage. Regenerating `cars.json` changed **zero sprite pixels** — `golden-sprites.test.ts` confirms it. **Applied as DERIVED values at `RaceField`'s three existing hooks, never by widening every rule:** contact perks become an **effective mass** passed to the untouched `resolveCarContact` (so a heavier car both shoves harder and is shoved less, and momentum is still conserved — measured to 1e-6), the dirt perk is an injected `SurfaceAdjuster` strategy so `OnTrackStep` still knows nothing about perks, and damage perks use a new optional `damageScale` on `applyImpactDamage` (a multiplier on the damage, NOT a bump to `armor`, which is clamped 0..1 and would have saturated for havac — the very car the perk exists for). **Measured: Off-road Ace covers 40.71 u on dirt vs 23.59 u (1.73x) yet stays below its 47.72 u on tarmac, so cutting is still a judgement call; Anvil's own contact ΔV is 5.0 vs 10.7 while its victim's rises to 20.0; Bulldozer sits between them at 7.35 and keeps 0.846 integrity as aggressor vs 0.744; a full draft lifts air-blade's terminal speed 95.0 -> 108.3; Trench Grip lifts battle-trak's drift threshold 2.417 -> 3.504 while braking and nothing off the brake.** A structural test asserts `CarPerk.ts` contains no assignment to velocity/position/heading, so decision 12's guarantee is actually checked. **`battle-trak` got `TRENCH_GRIP`, not `ARSENAL`:** with no weapons until T-016 there is no ammo consumption and no reload anywhere in `src/`, so an Arsenal perk would be a flag changing nothing a driver can feel. `ARSENAL` stays defined and **pinned inert by a test** for that day. **T-038 is the other half of the difficulty and T-037 alone will not close the owner's complaint.** |
| T-038 | **Make the NPCs race instead of commute** — racing line, overtaking, defending | `review` | NPC AI V2 `292c0ab` | T-014, T-037 | **NPC AI V2 shipped (decision 30).** Do not rewrite `RacingAgent` from this old PaceDriver brief. Live pipeline: corridor futures → `stepVehicle` rollout → RaceCore + personality bias; recover is orthogonal. Post-change mix 2:2:2 on Basin II: 0/6 labelled RECOVER (was 5/6). Remaining: ramp `s≈280` / airborne hang, then profile traits last. **Gate still open:** owner at the wheel should not win first attempt. |
| T-048 | Procedural art upgrade (ground tiles v2 + placeholder select art for planets 2-10) and themed trackside borders/props per planet | `done` | main (orchestrator) | T-036 | **Done 2026-08-16.** (a) `tools/art/generate-ground-tiles.ts` now draws a tileable Worley crack pattern, cellular blotches, grating/ripple streaks or staggered deck plating per planet instead of flat hash noise (`b6af9ed`). (b) `tools/art/generate-planet-select.ts` (new, `npm run gen:planet-select`) writes a banded-gradient-sky + silhouette stand-in for the 9 planets missing real Prompt A art (`429ce13`) — still explicitly placeholders, no image-gen tool is available here. (c) `PlanetTheme` gained `propShape/propHeight/propWidth/propColor/propAccent`; `TrackRenderer.drawBorderProps` stamps a boulder (`blob`) or a standing pipe/reed/pylon (`spike`, using `IsoProjection`'s height parameter) along both walls at a jittered arc-length interval (`bef2f16`) — this is the actual fix for "só aquela faixa branca não tá bom". All three verified by reading real screenshots, not object state; the first prop render was invisible because several planets' `propColor` exactly matched their `wall`/`shoulder` and had to be retuned for contrast. 914 tests, typecheck and build clean. |
| T-050 | Jump ramps: elevation + airtime | `in_progress` | main | — | **Arcade plan implemented 2026-08-18** (`docs/T-050-arcade-ramps.md`). `RampLaunch.resolveRampContact` = zone `launchSpeed` + car force at contact; 45° below 45% maxSpeed rejects into reverse; hot 85%+turbo uses the height/range table; mid-air turbo kicks once; 45° hot landing = 1s stun + 4% (`applyWeaponDamage`); void landing past the wall explodes and `sitOutWreck` respawns on the line after the ramp. Thunder Basin angles 45/30/15. Typecheck clean, 1155 tests. **`done` still waits on a screenshot of a car in the air on the bottom straight (decision 25).** Do not rewrite `integrateAirborne`, `AIRBORNE_SURFACE`, VehicleView, or the hop. |
...
| T-041 | **Race results screen: victory marker, total time, best-lap record** | `todo` | — | T-018 | **New scope from the owner, 2026-08-16: "não há marcação de vitória, tempo total, para completar a pista, quem já fez o menor tempo".** Today a race reaches `FINISHED` and simply sits there — `RaceSimulation` records `finishedAtSeconds` per racer and `PositionRanker` knows the order, but nothing tells the player they won. Needs: (a) a results screen after the last racer is home — final classification by display name, each racer's total time and the gap to the winner, and an unmistakable WIN/LOSE banner for the player; (b) **total time and best LAP time tracked per race** — `LapTracker` currently counts laps and gates but never times an individual lap, so a per-lap split has to be added to it (pure, and it takes elapsed seconds in, since `Date.now()` is forbidden); (c) **the track record** — "quem já fez o menor tempo" — persisted per track, which is exactly what T-035's `SaveSlots` already models (`best lap per track`), so this needs T-035's `CookieStore` to actually store it. Keep the classification maths pure in `src/domain/race/`; the screen is a scene like `SplashScene`. |
| T-042 | **Championship structure: 3 tracks per world, ordered by difficulty, with a points gate to the next world** | `todo` | — | T-041, T-036 | **New scope from the owner, 2026-08-16, explicitly deferred: "isso é para o próximo plano".** Verbatim intent: three tracks per world — 1 easy, 2 medium, 3 hard — and the player only advances to the next world on **80 points**, where the owner asked ME to work out what 1st/2nd/3rd is worth per track so it sums correctly, and also framed it as "80% do tempo mínimo". Today there is ONE track (`thunder-basin`) and `TRACKS`/`findTrack` is a flat registry with no world grouping, no difficulty ordering and no progression at all. **Proposed maths, CONFIRMED BY THE OWNER on 2026-08-16** — they read the proposal and said it is what they meant: *"T-042, follow as you said, are the same I did, just words put wrong way."* So **"80 points" and "80% do tempo mínimo" are ONE gate, not two**: a single 0–100 world score where 80 literally means 80%. Position component (70 of the 100) = points summed over the world's three tracks divided by the maximum, with a five-car race paying **1st 10, 2nd 6, 3rd 4, 4th 2, 5th 1** (30 available across three tracks); time component (the other 30) = the mean of `parTime / playerTime` per track, capped at 1, where each `TrackDefinition` carries an authored `parTime`. Three wins with poor times, or two wins and a second with good times, both clear 80 — winning matters most but cannot be the whole story. **Do NOT re-open this design; it is settled. What is NOT settled is the schedule: the owner deferred BUILDING it ("isso é para o próximo plano").** Needs a `world` grouping and a `difficulty` + `parTime` on `TrackDefinition`, the nine missing tracks (the ten planet illustrations from T-034 are the worlds), and progression persisted through T-035's save. Needs a `world` grouping and a `difficulty` + `parTime` on `TrackDefinition`, the nine missing tracks (the ten planet illustrations from T-034 are the worlds), and progression persisted through T-035's save. |
...
| T-029 | Tyre marks read too thick on screen | `done` | tyremark-width | T-022 | `STROKE_WIDTH_UNITS` in `TyreMarks.ts` halved 1.6 → 0.8 on the user's judgement at the wheel, 2026-08-15. Emission conditions, fade, segment cap, colour and the 0.9 wheel-track separation all untouched. **Closed 2026-08-16, implicitly rather than explicitly:** the owner has now driven several laps with the halved width while confirming T-039 and raised no objection to the thickness. Flagged as implicit acceptance so nobody mistakes it for a direct sign-off — if the marks ever read wrong again, `STROKE_WIDTH_UNITS` in `TyreMarks.ts` is the one knob. |
...
| --- | --- | --- | --- | --- | --- |
| main (orchestrator, Claude session 2026-08-16 ~05:30) | T-040, T-048, T-050 | `in_progress` | **T-040 done** (`25bd7a9`/`a118775`), **T-048 done** (`b6af9ed` ground tiles v2, `429ce13` placeholder select art, `bef2f16` trackside props) — all pushed except the last. **`git push origin main` is now blocked by the session's Apple sandbox domain allowlist** (github.com CONNECT 403) after working fine for the first three pushes this session; `bef2f16` is committed locally, not lost, just not on origin yet. Escalated to the owner with the three unblock options (dashboard, user allowlist, project allowlist) — not editing the allowlist myself, that is a security-boundary call for the owner. **Not writing `deploy.now` until a push actually lands**, per the owner's own instruction to only request a deploy after commit AND push. Continuing with T-050 (ramps) in the meantime since it has no push dependency. **Blocked by:** github.com push access. 2 attempts made, both failed identically; stopped retrying per protocol. | `WORKLOG.md` | 2026-08-16 06:05 |
...
**The owner's instruction this turn: do NOT start implementing; record the items and stop, because the
context is heavy.** T-018 was begun and then deliberately **halted mid-flight** — read the next
section before touching it.

**Verified state: 640 tests across 26 files, typecheck clean, build clean, everything pushed to
`https://github.com/klyff/game-race-90s`.**

#### T-018 was started and stopped — what is on disk

The `splash-scene` agent was killed part-way. It left exactly ONE finished file:
**`src/adapters/render/BlinkClock.ts` — kept, because it is pure, self-contained and correct in
intent**, but **it has NO TEST FILE yet** and is imported by nothing. `SplashScene.ts` was never
written; `BootScene`, `sceneKeys.ts` and `main.ts` were never touched, so the game still boots
straight into `RaceScene`. **Nothing is half-wired — the repo is consistent.** Whoever resumes T-018
should write `tests/adapters/BlinkClock.test.ts` first, then the scene.

#### Delivered this turn

| Item | State |
| --- | --- |
| **T-039 tyre marks** | **Diagnosed and fixed.** See its task row for the three-part root cause. The owner still needs to confirm it over a full lap. |
| `music.html` | **A listening bench for `TitleMusic`, at the project root.** The owner asked how to hear the music; the answer was that there was no way at all, because nothing plays it. `npm run dev` then http://localhost:5173/music.html gives a PLAY button. It exists only until the splash screen has a home for the music. |
| T-039 row, T-037/T-038 rows | Written into this ledger and the plan. |

#### Next steps, in order

1. **T-018 `SplashScene`** — the brief is in its task row and in the plan. `BlinkClock.ts` is already
   there; give it tests. **Draw NO title (the logo is painted into the art), and the blink is a HARD
   on/off cut via `setVisible`, never an alpha tween.** Music must start on the first KEY PRESS, not
   in `create()`, or the browser silently never sounds it — and it must be stopped on scene SHUTDOWN
   or it plays over the race.
2. **T-037 + T-038 together** — the difficulty. Ship both or neither; T-038's gate is the owner
   losing a race.
3. T-035's `CookieStore` + slot screen; then T-036 (`theme` on `TrackDefinition`).

#### The trap that produced T-039, stated plainly because it will happen again

**An adapter that holds per-entity state becomes a landmine the day the entity count goes from one to
many, and nothing in its API says "one".** `TyreMarks.record(state, telemetry, dt)` looked perfectly
...
### Context cleanup — 2026-08-16 00:15 — owner signed off T-039; save point

**T-039 is SIGNED OFF at the wheel: "o pneu voltou, ok na T-039".** T-029 closed with it, implicitly —
the owner drove several laps with the halved mark width and raised no objection.

**Nothing is now waiting on a human.** Every gate that needed the owner's eye or ear is closed:
handling and the five cars feeling different, the race audio, the title music, and both tyre-mark
items.

**Verified state: 640 tests across 26 files, typecheck clean, build clean, all pushed to
`https://github.com/klyff/game-race-90s`.** No agent is mid-flight.

#### The next session starts here, nothing blocking

1. **T-018 `SplashScene`** — the headline task; the owner has expected to see it twice. Brief is in its
   task row. `src/adapters/render/BlinkClock.ts` already exists (pure, correct in intent) but has **no
   test and is imported by nothing** — write `tests/adapters/BlinkClock.test.ts` first. **No drawn
   title** (the logo is painted into the art), blink is a **hard on/off cut via `setVisible`**, music
   starts on the first KEY PRESS and must be stopped on scene SHUTDOWN or it plays over the race.
   `music.html` at the root is the listening bench and can go once the splash owns the music.
2. **T-037 + T-038 together** — the difficulty, the owner's main open complaint. Gate: the owner races
   and does NOT win first try.
3. **T-035** `CookieStore` + slot screen → **T-036** `theme` on `TrackDefinition` → **T-040** ten music
   scores as DATA over one extracted player.

#### Standing lessons, all already paid for in this project

- **Read the screenshot, never object state** — `tools/verify/README.md` has the working recipe.
- **Subagent reports are claims, not facts.** Two died to API timeouts having written nothing; one
  shipped tests that passed while nothing worked; one a dimensionally wrong speed law; one an inverted
  label.
- **An adapter caching per-entity state breaks silently the day there is more than one entity** — that
  was T-039, and nothing in the old `TyreMarks` API said "one car".
- **`Date.now()`, `new Date()`, `Math.random()` are forbidden.** Assets live under `public/`, loaded by
  key, never `import`ed.

---

### T-019 (unchanged, kept for ordering)

| ID | Title | Status | Owner | Blocked by | Next step |
| --- | --- | --- | --- | --- | --- |
| T-019 | README, delivery report, `npm run build` verified | `todo` | — | T-018 | `npm run build` already verified clean during T-009 and again in T-018; README and the delivery report remain. |

---

### 2026-08-16 — T-018 delivered: the game opens on the splash screen

**Branch `main`. 704 tests across 29 files (was 640/26), typecheck clean, build clean.**

**Delivered.** The splash screen with car select, which the owner had expected to see twice.

- New: `src/scenes/SplashScene.ts`, `src/adapters/audio/TitleAudio.ts`,
  `src/adapters/render/SplashLayout.ts` (pure, 19 tests), `src/adapters/render/CarStatBars.ts`
  (pure, 12 tests), `tests/adapters/BlinkClock.test.ts` (33 tests for the file left over from the
  halted attempt).
- Changed: `sceneKeys.ts` (`SPLASH`, `SPLASH_ART_KEY`, `SPLASH_ART_FILE`, `UI_ASSET_DIRECTORY`),
  `BootScene.ts` (loads the splash art in the second pass, starts `SPLASH` instead of `RACE`),
  `main.ts` (registers `SplashScene`), `tools/verify/probe.mjs` (takes a scene key).
- **Verified by reading five screenshots, not object state**: the blink is a hard cut (absent at
  2400 ms, present at 2800 ms), LEFT/RIGHT swaps the car and its bars, SPACE starts the race with
  the chosen car.

**Still open.**
- **The title music has never been heard in the game.** An orchestrator cannot hear, and the
  headless browser has no audio device. Only the owner can close this one.
- T-037 + T-038, the difficulty, are next and unblocked.
- T-041 and T-042 are new scope from the owner this session; T-042 is explicitly for the next plan
  and carries one question that needs answering before it is built (see its row).

---

### Context cleanup — 2026-08-16 00:55 — context reached 312k, past the 290k ceiling

**Verified by running the commands, not by trusting any number written above: 854 tests across 32
files, `npm run typecheck` clean, `npm run build` clean.** T-018 is committed and pushed
(`768e377`); T-037's work is on disk and green but **not yet committed** at the moment of this dump.

#### What the owner asked for this session, in their own words

1. **"Go and execute the mission"** — T-018 was the headline and it is now DONE and pushed.
2. **"Falta passar de pista por pista … não há marcação de vitória, tempo total … quem já fez o menor
   tempo … 3 pistas por mundo … só vai para a próxima se conseguir 80 pontos"**, then **"isso é para o
   próximo plano"**. Recorded as **T-041** (results screen) and **T-042** (championship structure).
   **T-042 is NOT to be built yet** and carries one open question — see its row.
3. **"We still haven't a Velocimeter on the HUD/Screen! Should have long ago"**, then a Top Gear
   reference image: **"o Velocimetro, podemos pensar em algo parecido com TopGear"**.
4. **"ultracode"** — maximum thoroughness is authorised; token cost is not a constraint.

#### Agents involved this round

| Agent | Task | State at cleanup |
| --- | --- | --- |
| main (orchestrator) | T-018, T-037, speedo | Wrote `SplashScene` + `TitleAudio`, chained boot -> splash -> race, verified by reading FIVE screenshots, shipped and pushed T-018. Then wrote `CarPerk.ts` and wired all three perk hooks into `RaceField`'s locked step order, added `SurfaceAdjuster` to `OnTrackStep` and an optional `damageScale` to `applyImpactDamage`, added `CAR_PERK.TRENCH_GRIP`, reassigned battle-trak and regenerated `cars.json`. |
| blinkclock-test | T-018 | `done`. 33 tests for the orphaned `BlinkClock`. Found no defect in it. |
| splash-layout | T-018 | `done`. `SplashLayout.ts` + 19 tests: COVER scaling and anchors against the IMAGE's rect. |
| car-stat-bars | T-018 | `done`. `CarStatBars.ts` + 12 tests, roster-normalised bars. |
| perk-plumbing | T-037 | `done` and **independently checked**: all five cars print a real perk out of `cars.json`, and `golden-sprites.test.ts` still passes, so not one sprite pixel moved. |
| slipstream | T-037 | `done`. `Slipstream.ts` + 12 tests. Ramp verified by hand: 0 at gap 2.5, peak 1.0 at 6, 0 at 16. |
| hud-speed | speedo | `done`. `speed`/`maxSpeed` on `HudReadout`, `formatSpeed`, `speedFraction`, and a plain bar — **the bar is being REPLACED by the Top Gear gauge**. |
| seven-segment | speedo | `done`. `SevenSegment.ts` + 45 tests. **It caught a contradiction in its own brief** (`9 = abcfg` vs a required 6-segment count) and correctly chose the standard closed `9 = abcdfg`. |
| perk-outcome-tests | T-037 | `done`. 25 tests that measure OUTCOMES, not flags. No defects found. Numbers below. |
| speedo-gauge | speedo | **`in_progress` AT CLEANUP — the only agent mid-flight.** Building `src/adapters/render/SpeedoGauge.ts`, renaming `KPH_PER_WORLD_UNIT` to `MPH_PER_WORLD_UNIT`, adding `formatSpeedDigits` + `speedDigits` to `HudFormat`, and replacing the plain cyan speed bar in `HudScene`. **Whoever picks this up: check those four files for a half-written state, then BUILD AND READ A SCREENSHOT of the bottom-right corner — it has never been seen.** |

#### T-037 is functionally complete. The measured outcomes, so nobody re-derives them

| Perk | Car | Measured effect |
| --- | --- | --- |
| Off-road Ace | dirt-devil | 2 s full throttle on dirt: **40.71 u vs 23.59 u** neutral (1.73x). Still below its 47.72 u on tarmac, so the penalty is REDUCED, never removed — cutting stays a judgement call. |
| Anvil | havac | Identical collision: own ΔV **5.0 vs 10.7** neutral; the opponent's **20.0 vs 14.3**. Momentum conserved to 1e-6 against the effective masses. |
| Bulldozer | marauder | ΔV **7.35**, between neutral and Anvil, as its description claims. As aggressor keeps **0.846** integrity vs 0.744 neutral. |
| Slipstream | air-blade | Terminal speed **95.0 -> 108.3** at full draft (+14%), 101.65 at half draft, 95.0 at zero draft. |
| Trench Grip | battle-trak | Drift threshold **2.417 -> 3.504** while braking, unchanged off the brake. At a lateral speed of 2.96 the neutral car slides and the perked car holds. |
| Arsenal | (unassigned) | **Inert, and pinned inert by a test** until T-016 exists. |

**The physics trap this cost, written down so it is not "simplified" back into a bug.** The drag
coefficient is DERIVED as `(enginePower - rollingResistance) / maxSpeed²` (locked decision 11), so
raising `enginePower` alone raises the derived drag by the same factor and **terminal speed does not
move at all** — a draft with better acceleration and identical top speed, which is a slipstream that
can never pull you past the car in front. `drivingStats` therefore scales `enginePower` AND `maxSpeed`
together. A test integrates to terminal speed with engine power alone raised 14% and asserts it stays
at exactly 95.0, so the trap cannot be reintroduced silently.

#### How the next agent continues

1. **Finish the speedometer.** Check `speedo-gauge`'s four files, then `npm run build` and
   `node tools/verify/screenshot.mjs "file://$PWD/dist/index.html" /tmp/speedo.png 5200 ArrowUp 2500`
   and **READ the bottom-right corner of the image.** Hold `ArrowUp` or the car never moves and the
   dial reads 0, which looks identical to a broken gauge. `probe.mjs` now takes a scene key.
2. **T-038 is the remaining half of the difficulty**, and T-037 alone will not close the owner's
   complaint. Order matters: **extract `PaceDriver`'s proven maths into shared `PursuitSteering` and
   `CornerSpeed` FIRST**, so locked decision 27's three paid-for mistakes cannot be re-broken in a
   copy, then compose `AIDriver` from them. `RaceField.commandFor` must hand the driver a distilled
   rival view built from LIVE stage-1 `distance` values — **never `standings`, which stage 5 computes
   and is therefore one step stale.** `tests/tuning/LapTimes.test.ts` drives ONE car with no
   `RaceField` and cannot show an overtake, so a new multi-car harness is required.
   **Gate: the owner races and does NOT win on the first attempt.**
3. **T-041**, then **T-042** — but T-042 only after the owner answers whether the points gate and the
   time gate are one score or two.
4. **Still open and only the owner can close it: nobody has ever heard `TitleMusic` in the game.**
   An agent cannot hear and the headless browser has no audio device. Ask them to run `npm run dev`.

#### Standing lessons, every one already paid for here

- **Read the screenshot, never object state** — and this session showed why BOTH are needed: the splash
  prompt looked missing until a probe found it at (699, 449) `visible: true`, and two captures 400 ms
  apart proved it was simply mid-blink. State alone would have called a feature a bug; the image alone
  would have called a bug a feature.
- **Subagent reports are claims.** One agent this round found a genuine contradiction in its own brief
  and was right to override it; the numbers above were all re-read rather than taken on trust.
...
### Session start — 2026-08-16 ~05:00 — Claude team picks up T-034 (planet maps and art)

**Task per `docs/CLAUDE_HANDOFF.md`:** wire real Prompt A illustrations for planets 2–10,
replace stand-in ground tiles with real Prompt B tiles, tighten `planetThemes.ts` palettes,
optionally place Prompt C props. Read the handoff, the art brief, `planetThemes.ts` and
`TrackRenderer.ts` in full before starting.

**Blocked by:** the `Bash` tool is completely non-functional in this session. Every invocation,
including trivial ones (`echo test`), fails identically:
```
Exit code 127
zsh:1: no such file or directory: /Users/klyffharlley/.claude/apple/scripts/shell-command-checker-wrapper.sh
```
This is a `PreToolUse` hook (likely in `~/.claude/settings.json`) that shells every Bash call
through a wrapper script that no longer exists on disk. Attempts: 4 (plain command, with
`dangerouslyDisableSandbox: true`, direct path, and via a delegated Explore subagent — all
identical failure). Per the bounded-retry rule this is now escalated rather than retried further.

**Consequence — nearly everything in this task is blocked:**
- `npm test && npm run typecheck && npm run build` cannot run — no way to verify any change.
- `npm run gen:ground` / `gen:tracks` / `gen:lines` / `gen:track` cannot run.
- No `ls`/`git status`/directory listing — had to probe file existence one path at a time via `Read`.
- No image-generation tool is available in this session, and no image-crop/resize tool either
  (no ImageMagick/sharp path without Bash) — so real Prompt A/B assets cannot be produced or
  extracted from the contact sheets this session, even though both contact sheets DO exist and
  DO contain finished art for all 10 planets (`docs/art-briefs/references/contact-sheet-*.png`,
  confirmed by reading them as images).

**Exact ask for the owner:** check `~/.claude/settings.json` (and this repo's `.claude/settings*.json`,
which do not exist yet) for a hook wired to
`/Users/klyffharlley/.claude/apple/scripts/shell-command-checker-wrapper.sh` and either restore that
script or remove/disable the hook entry. Nothing in this task can proceed past palette tweaks until
Bash works again.

**What was NOT touched, and why:** did not edit `planetThemes.ts` colours this session. The file's
own header already says its palette was "pulled from the commissioned contact sheets" in T-036, and
on visual inspection against both contact sheets the existing values already look reasonably close.
Changing them further by eye, with no `typecheck`/`build` available to catch a mistake and no way to
screenshot the result to judge it, was judged not worth the risk of a silent, unverifiable regression.

**Next step for whoever picks this up:** once Bash works again, in this order:
1. Re-run `npm test && npm run typecheck` to confirm the baseline (887 tests / 35 files last verified,
   2026-08-16 ~03:35 — trust the repo over this number).
2. Crop Prompt A (16:9) and Prompt B (1024²) cells for planets 2–10 out of
   `docs/art-briefs/references/contact-sheet-tiles-and-props.png` (higher-res per-cell than the
   illustrations sheet) — this is the "last resort" the handoff explicitly allows since no
   image-gen tool is available. Verify Prompt B crops tile seamlessly (3×3 tile + look for a seam)
   before dropping them at `public/assets/ground/<slug>.png`.
   Drop Prompt A crops at `public/assets/ui/planets/<slug>.jpeg`.
